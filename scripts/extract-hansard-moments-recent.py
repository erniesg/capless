#!/usr/bin/env python3
"""
Extract moments from MOST RECENT 100 Hansard transcripts only
Preserves section-based chunking logic
"""
...
def chunk_old_format(full_text):
    """Chunk old format by paragraph boundaries"""
...
def main():
    print("=== HANSARD MOMENT EXTRACTION (ALL SESSIONS) ===", file=sys.stderr)
    print(f"Start: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", file=sys.stderr)
    print("", file=sys.stderr)

    all_dates = list_hansard_transcripts()

    # Process ALL dates (including those with YouTube transcripts)
    # For dates with both YouTube + Hansard, we'll have 2 sets of moments
    dates = all_dates

    print(f"", file=sys.stderr)
    print(f"📋 Processing ALL {len(dates)} Hansard transcripts (reverse chronological)", file=sys.stderr)
    print(f"   Date range: {dates[-1]} to {dates[0]}", file=sys.stderr)
    print(f"   Note: Dates with YouTube transcripts will have both video-based and Hansard-based moments", file=sys.stderr)
    print("", file=sys.stderr)
